...
def read_embedding(
    ...
    if get_marker_significance:
        path_emb_marker = os.path.join(
            Path(path_emb).parent.as_posix() + "_with_sig", os.path.basename(path_emb)
        )
        path_entity_marker = os.path.join(
            Path(path_entity).parent.parent.as_posix() + "_with_sig", "input/entity"
        )
    ...
    for x in os.listdir(path_emb):
        if x.startswith("embeddings"):
            ...
            if (len(prefix) == 0) or (entity_type in prefix):
                adata = read_hdf(
                    os.path.join(
                        path_emb, f"embeddings_{entity_type}_0." f"v{num_epochs}.h5"
                    ),
                    key="embeddings",
                )
                with open(
                    os.path.join(path_entity, f"entity_names_{entity_type}_0.json"),
                    "rt",
                ) as tf:
                ...
                if get_marker_significance:
                    try:
                        dict_adata[f"n{entity_type}"] = dict_adata_with_sig[
                            f"n{entity_type}"
                        ]
                    except KeyError:
                        print(
                            f"Null feature nodes for entity {entity_type} not embedded."
                        )
...
def write_bed(adata, use_top_pcs=True, filename=None):
    ...
    if use_top_pcs:
        assert (
            "top_pcs" in adata.var_keys()
        ), "please run `si.pp.select_pcs_features()` first"
        peaks_selected = adata.var[adata.var["top_pcs"]][["chr", "start", "end"]]
    else:
